...
select subjects
```{r}
n_try = 50#  number of subjects to try
...
plot(precis(fit.choice.nopool.ulam))
...
fit.choice.partpool.factor.2 <- map2stan(
  alist(
    choice ~ dbinom(1, choice_prob), # sample from bernoulli distribution
    logit(choice_prob) <- (uL-uR)*slope[id],
    uL <- (prob - beta[id] * (ambig/2)) * val ^ (slope_age*age[id] + miu_a), # subjective value of the lottery
    uR <- (prob_ref - beta[id] * (ambig_ref/2)) * val_ref ^ (slope_age*age[id] + miu_a), # subjective value of the reference
