...
```{r}
setwd("E:/Ruonan/Projects in the lab/mdm_covid19/behavior/mdm_covid19_data")
path = "E:/Ruonan/Projects in the lab/mdm_covid19/behavior/mdm_covid19_data"
...
for (id in ids) {
  ...
  p1 <- ggplot(data_plot, aes(x=val, y=prob, fill=choice)) + 
    ...
    scale_x_discrete(breaks = c(500,800,1200,2500)) +
    scale_fill_gradient(limits = c(0,1)) +
    theme(panel.grid = element_blank(),
          panel.background = element_blank()) +
    ggtitle(paste("Monetary Risky, Sub", as.character(id))) +
    xlab("Value ($)") + ylab("Probability Level")
  ...
  p3 <- ggplot(data_plot, aes(x=val, y=prob, fill=choice)) + 
    ...
    theme(panel.grid = element_blank(),
          panel.background = element_blank()) +
    ggtitle("Medical Risky") +
    xlab("Outcome") + ylab("Probability Level")
...
## correlation between risk and ambiguity choices
```{r}
ggplot(data.attitude[data.attitude$error.mon<0.5,], aes(x=risk.mon, y=ambig.mon)) +
  geom_point() +
  geom_smooth(method="lm")

cor.test(data.attitude[data.attitude$error.mon<0.5,]$risk.mon,
         data.attitude[data.attitude$error.mon<0.5,]$ambig.mon)
```

```{r}
ggplot(data.attitude[data.attitude$error.mon<0.5,], aes(x=risk.mon, y=ambig_corr.mon)) +
  geom_point() +
  geom_smooth(method="lm")

cor.test(data.attitude[data.attitude$error.mon<0.5,]$risk.mon,
         data.attitude[data.attitude$error.mon<0.5,]$ambig_corr.mon)

nrow(data.attitude[data.attitude$error.mon<0.5,])
```

```{r}
ggplot(data.attitude[data.attitude$error.med<0.5,], aes(x=risk.med, y=ambig.med)) +
  geom_point() +
  geom_smooth(method="lm")

cor.test(data.attitude[data.attitude$error.med<0.5,]$risk.med,
         data.attitude[data.attitude$error.med<0.5,]$ambig.med)
```

```{r}
ggplot(data.attitude[data.attitude$error.med<0.5,], aes(x=risk.med, y=ambig_corr.med)) +
  geom_point() +
  geom_smooth(method="lm")

cor.test(data.attitude[data.attitude$error.med<0.5,]$risk.med,
         data.attitude[data.attitude$error.med<0.5,]$ambig_corr.med)

nrow(data.attitude[data.attitude$error.med<0.5,])
```
...
select factor
```{r}
factor <- data.all %>% select(age, mTurkCode)
factor
factor<-rename(factor, id=mTurkCode)
typeof(factor$id)
typeof((data.choice.mon$id))

factor <- factor %>% mutate(id=as.integer(as.character(id)))
data.choice.mon <- merge(data.choice.mon, factor, by="id")
...
#### select subjects
```{r}
n_try = 150#  number of subjects to try
...
data_list <- list(
  ...
  age = data.choice.mon$age[(no_trials*(start-1) + 1):((start + n_try-1)*no_trials)],
...
```{r}
par.nopool <- data.frame(precis(fit.choice.nopool.ulam, depth=2))
# str(fit.choice)
plot(fit.choice.nopool)
traceplot(fit.choice.nopool.ulam)


plot(precis(fit.choice.nopool.ulam, depth=2))
View(fit.choice.nopool.ulam)
# View(par.nopool)
```
...
Look at distributions for setting up priors
```{r}
y<-rexp(1000,1)
hist(y)
# hist(log(y))
...
fit.choice.partpool <- map2stan(
  ...
  data = data_list, iter = 1e3, chains=4,
  start=list(miu_a=0.5, miu_b=0.5, miu_s=1, sigma_a = 1, sigma_b = 1, sigma_s = 1),
  constraints=list(alpha="lower=0", miu_a="lower=0"),
  control=list(adapt_delta=0.95)
)
```
...
fit.choice.partpool.repar <- map2stan(
  ...
  data = data_list, iter = 1e3, chains=4,
  start=list(miu_a=0.3, miu_b=0.5, miu_s=1, sigma_a = 1, sigma_b = 1, sigma_s = 1),
  control=list(adapt_delta=0.95)
)
...
traceplot(fit.choice.partpool)
plot(precis(fit.choice.partpool, depth=2))
...
Look at model fitting result
```{r}
paropool <- data.frame(precis(fit.choice.nopool.ulam, depth=2))
# str(fit.choice)
plot(fit.choice.nopool)
traceplot(fit.choice.nopool.ulam)
...
partpool <- data.frame(
  id=c(1:n_try),
  alpha=par.partpool$mean[(1+6):(n_try+6)],
  beta=par.partpool$mean[(n_try + 1+6):(2*n_try+6)],
  slope=par.partpool$mean[(2*n_try + 1+6):(3*n_try+6)],
  method=rep('partpool', n_try)
)

# re-parameterized model, partial pooling
# map2stan output
...
ggplot(compare[is.element(compare$id, seq(1,n_plot)),], aes(x = id, y = alpha, color = method, fill = method)) + 
  geom_point(size = 3, alpha = 0.5)

ggplot(compare[is.element(compare$id, seq(1,n_plot)),], aes(x = id, y = beta, color = method, fill = method)) + 
  geom_point(size = 3, alpha = 0.5)

ggplot(compare[is.element(compare$id, seq(1,n_plot)),], aes(x = id, y = slope, color = method, fill = method)) + 
  geom_point(size = 3, alpha = 0.5)
...
```{r}
id_match <- unique(select(data.choice.mon, c("id", "subid")))
id2compare <- id_match$id[1:n_try]
id2compare
data.attitude

choice_mf <- data.attitude[is.element(data.attitude$mTurkCode, id2compare), ]

# check if ids match
plot(as.numeric(as.character(id2compare)), as.numeric(as.character(data.attitude$mTurkCode[is.element(data.attitude$mTurkCode, id2compare)])))
     
plot(choice_mf$risk.mon, compare$alpha[compare$method=="nopool"],
     xlab="alpha", ylab="model-free risk attitude")

plot(choice_mf$ambig_corr.mon, compare$alpha[compare$method=="nopool"],
     xlab="beta", ylab="model-free ambiguity attitude")
```

## Age multilevel model
using ulam
```{r}

fit.choice.partpool.factor <- ulam(
  alist(
    choice ~ dbinom(1, choice_prob), # sample from bernoulli distribution
    logit(choice_prob) <- (uL-uR)*slope[id],
    uL <- (prob - beta[id] * (ambig/2)) * val ^ (slope_age*age[id] + miu_a + alpha[id]), # subjective value of the lottery
    uR <- (prob_ref - beta[id] * (ambig_ref/2)) * val_ref ^ (slope_age*age[id] + miu_a + alpha[id]), # subjective value of the reference
    
    beta[id] ~ dnorm(miu_b, sigma_b),
    slope[id] ~ dnorm(miu_s, sigma_s),
    alpha[id] ~ dlnorm(-0.5, 0.5),
    
    # group level priors
    # priors for age effect on risk attitude
    slope_age ~ dnorm(0, 1),
    miu_a ~ dnorm(0.5, 1),
    # sigma_a ~ dexp(2),
  ...
  data = data_list, iter = 1e5, chains=3,
  start=list(slope_age=0, miu_a = 0.8, miu_b=0.7, miu_s=0.8, sigma_b = 0.5, sigma_s = 0.5),
  control=list(adapt_delta=0.95)
)
...
Look at model fitting result
```{r}
par.partpool.factor <- data.frame(precis(fit.choice.partpool.factor, depth=2))
View(par.partpool.factor)

age_n_try = data_list$age[seq(1,length(data_list$age),no_trials)]
  
partpool.factor<- data.frame(
  id=c(1:n_try),
  alpha=age_n_try * par.partpool.factor["slope_a", "mean"] + par.partpool.factor["miu_a", "mean"],
  beta=par.partpool.factor$mean[1:n_try],
  slope=par.partpool.factor$mean[(n_try + 1):(2*n_try)],
  method=rep('partpool.factor', n_try)
)

View(partpool.factor)
```
...
Use map2stan
```{r}
fit.choice.partpool.factor.2 <- map2stan(
  alist(
    choice ~ dbinom(1, choice_prob), # sample from bernoulli distribution
    logit(choice_prob) <- (uL-uR)*slope[id],
    uL <- (prob - beta[id] * (ambig/2)) * val ^ (slope_age*age + miu_a), # subjective value of the lottery
    uR <- (prob_ref - beta[id] * (ambig_ref/2)) * val_ref ^ (slope_age*age + miu_a), # subjective value of the reference
  ...
  data = data_list, iter = 1e5, chains=4,
  start=list(slope_age=0, miu_a = 0.5, miu_b=0.7, miu_s=0.8, sigma_b = 0.5, sigma_s = 0.5),
  control=list(adapt_delta=0.95)
)
```
...
```{r}
fit.choice.partpool.factor.2
View(fit.choice.partpool.factor.2)
fit.choice.partpool.factor.2$coef
plot(fit.choice.partpool.factor.2)
precis(fit.choice.partpool.factor.2)
```
