...
data_list <- list(
  ...
  id = data.choice.mon$id
)

n_try = 5
start = 11
data_list <- list(
  choice = data.choice.mon$choice[start:(start + n_try*no_trials)],
  prob = data.choice.mon$prob[1:n_try*no_trials],
  ambig = data.choice.mon$ambig[1:n_try*no_trials],
  val = data.choice.mon$val[1:n_try*no_trials],
  prob_ref = data.choice.mon$prob.ref[1:n_try*no_trials],
  ambig_ref = data.choice.mon$ambig.ref[1:n_try*no_trials],
  val_ref = data.choice.mon$val.ref[1:n_try*no_trials],
  id = data.choice.mon$subid[1:n_try*no_trials]
)
...
fit.choice.nopool <- ulam(
  alist(
    ...
    alpha[id] ~ dlnorm(0.5, 2),
    beta[id] ~ dnorm(0.2, 2),
    slope[id] ~ dnorm(1, 3)
    
  ), 
  data = data_list, iter = 1e3, chains=4,
  start = list(alpha = rep(0.5, n_try), beta = rep(0.2, n_try), sigma = rep(1, n_try))
)
...
fit.choice.partpool <- ulam(
  ...
  data = data_list, iter = 1e3, chains=4,
  start=list(miu_a=0.5, miu_b=0.2, miu_s=1, sigma_a = 1, sigma_b = 1, sigma_s = 1)
)
...
nopool <- data.frame(
  id = c(1:5),
  alpha = par.nopool$mean[1:5],
  beta = par.nopool$mean[6:10],
  slope = par.nopool$mean[11:15],
  method=rep('nopool', 5)
)

partpool <- data.frame(
  id=c(1:5),
  alpha=par.partpool$mean[1:5],
  beta=par.partpool$mean[6:10],
  slope=par.partpool$mean[11:15],
  method=rep('partpool', 5)
)
