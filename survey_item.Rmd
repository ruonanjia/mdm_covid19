...
# location of county by zipcode
data.zipcode<-aggregate(
  data.frame(anx=data.all$interest_in_politics)-4,list(zip=data.all$zipcode),
  mean,
  na.action=na.omit)
...
state_match <- match(geo$state, state_all)
for (idx in 1:length(state_all)) {
  geo_state$interest_in_politics[idx] <- mean(geo$interest_in_politics[geo$state==state_all[idx]],na.rm=TRUE)
}

# plot
plot_usmap(data=geo_state, regions="states",values="interest_in_politics") +
  scale_fill_gradient2(midpoint=0,low=muted("blue"),high=muted("red")) +
  ggtitle("How intereste are you in politics") +
  labs(fill = "Extremely (Red)\n~\nA fair amount (White) \n~\nNot at all (Blue)\n") +
  theme(legend.position = "right")
```
