...
## select Qs
```{r}
data.classify <- data.all %>% select(mTurkCode, age:gender,
                                     ...
                                     actions_household:actions_child,
                                     actions_adult70,
...
```{r}
data.all$itemspurchased <- as.character(data.all$actions_itempurchase)
data.all$itemspurchased <- strsplit(data.all$itemspurchased, ",")
data.all$itemspurchased <- lengths(data.all$itemspurchased)
```
...
```{r}
ncol(data.classify)
temp1 <- data.classify %>% mutate_at(3:ncol(data.classify), as.character)
temp2 <- temp1 %>% mutate_at(3:ncol(data.classify), as.integer)
temp1
```
