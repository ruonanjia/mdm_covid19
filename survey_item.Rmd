...
cor.test(data.all$age, data.all$ambig_score)


cor.test(data.all$age[data.all$is.young==1], data.all$ambig_score[data.all$is.young==1])
cor.test(data.all$age[data.all$is.young==0], data.all$ambig_score[data.all$is.young==0])

```

general code
```{r}
colnames(data.all)[407:411]
y2plot = "infect"

ggplot(data.all, aes(x=age, y=eval(parse(text=y2plot)))) + 
  geom_point()+
  geom_smooth(method="lm") +
  xlab("Age") + ylab(y2plot)

# separate young and old
ggplot(data.all, aes(x=age, y=eval(parse(text=y2plot)), color=is.young)) + 
  geom_point()+
  geom_smooth(method="lm") +
  xlab("Age") + ylab(y2plot)
...
```{r}
mask_med <- data.all$error.med < 0.5 & data.all$risk.med != 0 & data.all$risk.med !=1
mask_mon <- data.all$error.mon < 0.5 & data.all$risk.mon != 0 & data.all$risk.mon !=1

mask <- data.all$error.med < 0.5 & data.all$risk.med != 0 & data.all$error.mon < 0.5 & data.all$risk.mon != 0 & data.all$risk.med !=1 & data.all$risk.mon != 1
sum(mask_med)
sum(mask_mon)
sum(mask)
```
...
# note here, correlation effect different in age separated
...
cor.test(data.all$ambig_score[mask_med & data.all$is.young==0], data.all$risk.med[mask_med & data.all$is.young==0])
...
correlation test
```{r}
test_corr <- cor.test(~ambig_corr.med + ambig_score, 
                      data=data.all[mask_med & mask_mon,])

text = print(test_corr)
...
ggplot(score_tb, aes(x = PC1, color=is.young, fill=is.young)) +
  ...
  scale_fill_discrete(name="Age", labels=c('Older (above and including 60)', 'Younger (below 60)')) +
  scale_color_discrete(name="Age", labels=c('Older (above and including 60)', 'Younger (below 60)')) +
  theme_classic() +
  theme(text = element_text(size=12), axis.line = element_line(size = 1),
        axis.ticks = element_line(colour = "black", size = 1),
        axis.text = element_text(colour = "black", size = 10),
...
```{r}
temp <- data.all %>% filter(error.mon<0.5 & error.med<0.5) %>% mutate(ambig1_news_perday=as.numeric(as.character(ambig1_news_perday)))

ggplot(temp, aes(x=ambig1_news_perday, y=ambig_corr.med)) +
  geom_point()

cor.test(temp$ambig1_news_perday, temp$ambig_corr.med)
```
...
[341] "belief_in_God"                "religiosity_rating"          
[343] "OUS1"                         "OUS2"                        
[345] "OUS3"                         "OUS4"                        
[347] "OUS5"                         "OUS6"                        
[349] "OUS7"                         "OUS8"                        
...
```{r}
# names(data.all)
data.all$interest_in_politics <- as.integer(as.character(data.all$interest_in_politics))

# some NA values
# data.all$interest_in_politics[435]

labels <- c("Very uninterested", "Somewhat uninteresed", "Slightly uninterested", "Not sure", "Slightly interested", "Somewhat interested", "Very interested")

ggplot(data.all, aes(x=interest_in_politics, fill = is.young)) +
  geom_histogram(stat="count", position="stack") +
  scale_x_continuous(breaks = seq(1,7,1), labels=labels) +
  rotate_x_text(30) +
  scale_fill_discrete(labels=c("Older", "Younger"), name="") +
...
### map
```{r}
data.all$interest_in_politics <- as.numeric(as.character(data.all$interest_in_politics))

# location of county by zipcode
data.zipcode<-aggregate(
  data.frame(pol_interest=data.all$interest_in_politics)-4,list(zip=data.all$zipcode),
  mean,
  na.action=na.omit)
...
for (idx in 1:length(state_all)) {
  geo_state$pol_interest[idx] <- mean(geo$pol_interest[geo$state==state_all[idx]],na.rm=TRUE)
}

# plot
plot_usmap(data=geo_state, regions="states",values="pol_interest") +
  scale_fill_gradient2(midpoint=0,low=muted("blue"),high=muted("red")) +
  ggtitle("How interested are you in politics") +
  labs(fill = "Very interested (Red)\n~\nNot sure (White) \n~\nVery uninterested (Blue)\n") +
  theme(legend.position = "right")
```

### uncertainty attitude
```{r}
data.all$interest_in_politics <- as.factor(as.character(data.all$interest_in_politics))

labels <- c("Very uninterested", "Somewhat uninteresed", "Slightly uninterested", "Not sure", "Slightly interested", "Somewhat interested", "Very interested")

# scatter plot
ggplot(data.all[data.all$error.med<0.5 & !is.na(data.all$interest_in_politics),], aes(x=interest_in_politics , y=ambig_corr.med)) + 
  geom_point() +
  ylab("model-free ambig Medical")

# medical ambiguity, 7 factor
ggplot(data.all[data.all$error.med<0.5 & !is.na(data.all$interest_in_politics),], aes(x=interest_in_politics , y=ambig_corr.med)) + 
  geom_violin() +
  # geom_dotplot(binaxis='y', stackdir='center', dotsize=0.3) +
  xlab("Interest in politics") +
...
# medical ambiguity, collapse into groups
data2plot <- data.all %>% dplyr :: select(
  error.med, error.mon, risk.med, risk.mon, ambig_corr.med, ambig_corr.mon, interest_in_politics
  )
data2plot$interest_in_politics <- as.numeric(as.character(data2plot$interest_in_politics))

data2plot$interest_in_politics_group[data2plot$interest_in_politics < 4] <- -1
data2plot$interest_in_politics_group[data2plot$interest_in_politics > 4] <- 1
data2plot$interest_in_politics_group[data2plot$interest_in_politics == 4] <- 0
data2plot$interest_in_politics_group[is.na(data2plot$interest_in_politics)] <- NA

# data2plot$interest_in_politics_group_t <- dplyr :: case_when(
#   (data2plot$interest_in_politics < 4) ~ -1,
#   (data2plot$interest_in_politics > 4) ~ 1,
#   (data2plot$interest_in_politics == 4) ~ 0,
#   (is.na(data2plot$interest_in_politics)) ~ NA
# )

data2plot$interest_in_politics_group <- as.factor(as.character(data2plot$interest_in_politics_group))

labels <- c("Uninterested", "Not sure", "Interested")

ggplot(data2plot[data2plot$error.mon<0.5 & !is.na(data2plot$interest_in_politics),], aes(x=interest_in_politics_group , y=ambig_corr.mon)) + 
  geom_violin() +
  # geom_dotplot(binaxis='y', stackdir='center', dotsize=0.3) +
  xlab("Interest in politics") +
  rotate_x_text(30) +
  ...
  geom_jitter(shape=16, position=position_jitter(0.2),size=1,alpha=0.3,color="red") +
  geom_boxplot(width=0.1) +
  ylab("model-free ambiguity Monetary")

# t test
t.test(data2plot[data2plot$error.mon<0.5 & !is.na(data2plot$interest_in_politics) & data2plot$interest_in_politics_group == -1,]$ambig_corr.mon,
       data2plot[data2plot$error.mon<0.5 & !is.na(data2plot$interest_in_politics) & data2plot$interest_in_politics_group == 1,]$ambig_corr.mon
)
...
## political typology
```{r}
data.all$political_typology <- as.integer(as.character(data.all$political_typology))

unique(data.all$political_typology)

labels <- c("Very liberal", "Liberal", "Slightly liberal", "Moderate/middle-of-the-road", "Slightly conservative", "Conservative", "Very conservative", "Don't know/not political", "Libertarian")

ggplot(data.all, aes(x=political_typology, fill = is.young)) +
  geom_histogram(stat="count", position="stack") +
  scale_x_continuous(breaks = seq(1,9,1), labels=labels) +
  rotate_x_text(30) +
  scale_fill_discrete(labels=c("Older", "Younger"), name="") +
  xlab("Political typology") +
  ggtitle("Political typology")

```

map
```{r}
data.all$political_typology <- as.numeric(as.character(data.all$political_typology))

# location of county by zipcode
# need to exclude ansowers of 8 ("Don't know/not political) and 9 (Libertarian)
data.zipcode<-aggregate(
  data.frame(pol_type=data.all$political_typology[data.all$political_typology != 8 & data.all$political_typology != 9])-4,list(zip=data.all$zipcode[data.all$political_typology != 8 & data.all$political_typology != 9]),
  mean,
  na.action=na.omit)
...
for (idx in 1:length(state_all)) {
  geo_state$pol_type[idx] <- mean(geo$pol_type[geo$state==state_all[idx]],na.rm=TRUE)
}

# plot
plot_usmap(data=geo_state, regions="states",values="pol_type") +
  scale_fill_gradient2(midpoint=0,low=muted("blue"),high=muted("red")) +
  ggtitle("Political typology") +
  labs(fill = "Very conservative (Red)\n~\nMiddle-of-the-road (White) \n~\nVery Liberal (Blue)\n") +
  theme(legend.position = "right")
```

## believe in god
```{r}
data.all$belief_in_God <- as.integer(as.character(data.all$belief_in_God))

unique(data.all$belief_in_God)

labels <- c("Very much", "Moderately", "Slightly", "Not sure", "Slightly do not", "Moderately do not", "Do not at all", "Agnostic")

ggplot(data.all, aes(x=belief_in_God, fill = is.young)) +
  geom_histogram(stat="count", position="stack") +
  scale_x_continuous(breaks = seq(1,8,1), labels=labels) +
  rotate_x_text(30) +
  scale_fill_discrete(labels=c("Older", "Younger"), name="") +
  xlab("Believe in God") +
  ggtitle("Believe in God")

```

map
```{r}
data.all$belief_in_God <- as.numeric(as.character(data.all$belief_in_God))

# location of county by zipcode
# need to exclude ansowers of 8 (Agnostic)
data.zipcode<-aggregate(
  data.frame(belief_god=data.all$belief_in_God[data.all$belief_in_God != 8])-4,list(zip=data.all$zipcode[data.all$belief_in_God != 8]),
  mean,
  na.action=na.omit)
...
for (idx in 1:length(state_all)) {
  geo_state$belief_god[idx] <- mean(geo$belief_god[geo$state==state_all[idx]],na.rm=TRUE)
}

# plot
plot_usmap(data=geo_state, regions="states",values="belief_god") +
  scale_fill_gradient2(midpoint=0,low=muted("blue"),high=muted("red")) +
  ggtitle("How much do you believe in God") +
  labs(fill = "Do not at all (Red)\n~\nNot sure (White) \n~\nVery Much (Blue)\n") +
  theme(legend.position = "right")
```

## religiosity
```{r}
data.all$religiosity_rating <- as.integer(as.character(data.all$religiosity_rating))

unique(data.all$religiosity_rating)

labels <- c("Very", "Moderately", "Slightly", "Not sure", "Slightly not", "Moderately not", "Not at all")

ggplot(data.all, aes(x=religiosity_rating, fill = is.young)) +
  geom_histogram(stat="count", position="stack") +
  scale_x_continuous(breaks = seq(1,7,1), labels=labels) +
  rotate_x_text(30) +
  scale_fill_discrete(labels=c("Older", "Younger"), name="") +
...
map
```{r}
data.all$religiosity_rating <- as.numeric(as.character(data.all$religiosity_rating))

# location of county by zipcode
data.zipcode<-aggregate(
  data.frame(relig=data.all$religiosity_rating)-4,list(zip=data.all$zipcode),
  mean,
  na.action=na.omit)
...
for (idx in 1:length(state_all)) {
  geo_state$relig[idx] <- mean(geo$relig[geo$state==state_all[idx]],na.rm=TRUE)
}

# plot
plot_usmap(data=geo_state, regions="states",values="relig") +
  scale_fill_gradient2(midpoint=0,low=muted("blue"),high=muted("red")) +
  ggtitle("How religious are you") +
  labs(fill = "Not at all (Red)\n~\nNot sure (White) \n~\nVery (Blue)\n") +
  theme(legend.position = "right")
```
