...
data.demo <- data.all[,c(155:164,166:171, 1)]
...
data.zipcode<-aggregate(
  data.frame(health_impact=data.all$ambig2_health_impact),list(zip=data.all$zipcode),
  mean,
  na.action=na.omit)
...
plot_usmap(data=geo_state, regions="states",values="health_impact") +
  scale_fill_gradient(limits=c(1,5)) +
  ggtitle("Should consider health impact") +
  labs(fill = "Extremely(1) ~ Not at all(7)") +
...
# location of county by zipcode
data.zipcode<-aggregate(
  data.frame(economic_impact=data.all$ambig2_economics),list(zip=data.all$zipcode),
  mean,
  na.action=na.omit)
...
# location of county by zipcode
data.zipcode<-aggregate(
  data.frame(health_economic=data.all$ambig2_health_impact-data.all$ambig2_economics),list(zip=data.all$zipcode),
  mean,
  na.action=na.omit)
...
plot_usmap(data=geo_state, regions="states",values="health_economic") +
  ...
  ggtitle("Should consider impact: heath-economics contrast") +
  labs(fill = "Health(-) ~ Economic(+)") +
  theme(legend.position = "right")
...
# location of county by zipcode
data.zipcode<-aggregate(
  data.frame(informed=data.all$ambig1_gov_local),list(zip=data.all$zipcode),
  mean,
  na.action=na.omit)
...
# location of county by zipcode
data.zipcode<-aggregate(
  data.frame(react=data.all$ambig1_react_state),list(zip=data.all$zipcode),
  mean,
  na.action=na.omit)
...
plot_usmap(data=geo_state, regions="states",values="react") +
  ...
  labs(fill = "Overreacting(1) ~ Insufficient(7)") +
  theme(legend.position = "right")
...
# location of county by zipcode
data.zipcode<-aggregate(
  data.frame(anx=data.all$anx_feelingnow),list(zip=data.all$zipcode),
  mean,
  na.action=na.omit)
...
plot_usmap(data=geo_state, regions="states",values="anx") +
  ...
  labs(fill = "Not at all(1) ~ Extremely(7)") +
  theme(legend.position = "right")
