...
correct item actions_livealone, actions_houshold
```{r}
mask <- data.all$age >= 60

# should be actions_household, actions_live alone
# but for olde (>=60), it is switched
livealone_old <- data.all$actions_household[mask]
household_old <- data.all$actions_livealone[mask]

livealone_old
household_old

data.all$actions_livealone[mask] <- livealone_old
data.all$actions_household[mask] <- household_old
```
...
In the older survey, the order of actions_livealone and actions_household are switched, need to correct it.
```{r}
...
data.classify <- mutate_at(data.classify,
                           c("household.earnings", "medical.insurance",
                             ...
                             "highest_education", "feeling_right_now", 
...
df2classify <- dplyr :: select(
  ...
  -c(zip, mTurkCode,
     ...
     employment_before,employment_current,
     ethnicity, marital_status, field_of_education, state
     )
...
df2classify <- df2classify[complete.cases(df2classify), ]
